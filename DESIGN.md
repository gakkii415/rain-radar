...
- Product / users / primary task: 日本国内の一般利用者向け雨雲レーダー。地名入力から、その地点の現在〜最大15時間後を確認することが最優先。
...
- Information density / hierarchy: 地図を最大面積で表示し、場所検索、60分/15時間の範囲切替、時間操作を常時到達可能にする。出典と注意事項は補助階層。
...
- Durable design decisions: Hero、汎用カード群、背景装飾を置かず、画面の重心を地図に固定する。固有表現は「地図＋60分/15時間切替＋実況/何時間後＋実時刻」が一体になった下部タイムライン。
